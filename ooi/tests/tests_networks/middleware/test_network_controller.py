...
class TestNetworkController(test_middleware.TestMiddleware):
    ...
    def test_list_networks_empty(self, m):
        ...
        url = "/networks"
        req = self._build_req(url , method="GET", headers=headers, content_type="text/occi")
        m.return_value = collection.Collection(create_occi_results(fakes.networks[tenant['id']]))
    ...
    def test_show_networks(self, m):
        ...
        for network in fakes.networks[tenant["id"]]:
            m.return_value = create_occi_results([network])[0]
