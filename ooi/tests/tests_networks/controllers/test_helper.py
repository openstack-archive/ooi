...
from ooi.api.networks.helpers import OpenStackNet
...
class TestNetOpenStackHelper(base.TestCase):
    def setUp(self):
        ...
        self.helper = OpenStackNet( self.version)
    ...
    @mock.patch.object(OpenStackNet, "_make_get_request")
    ...
    @mock.patch.object(OpenStackNet, "_get_req")
    def test_index(self, m):
        resp = fakes.create_fake_json_resp({"networks": ["FOO"]}, 200)
        req_mock = mock.MagicMock()
        req_mock.get_response.return_value = resp
        ...
        ret = self.helper.index(None,None)
        self.assertEqual(["FOO"], ret)
        m.assert_called_with(None, method="GET", path="/networks", query_string=None)


    @mock.patch.object(OpenStackNet, "_make_get_request")
    def test_get_network(self, m):
        resp = fakes.create_fake_json_resp({"network": {"status":"ACTIVE"}}, 200)
        req_mock = mock.MagicMock()
        req_mock.get_response.return_value = resp
        m.return_value = req_mock
    ...
    def test_get_network2(self, m):
        ...
        ret = self.helper.get_network(None,1)
        self.assertEqual("ACTIVE", ret["status"])
        m.assert_called_with(None, method="GET", path="/networks/1", query_string=None)
    ...
    def test_get_network_with_subnet(self, m):
        resp = fakes.create_fake_json_resp({"network": {"status":"ACTIVE", "subnets":[2]}, "subnet":["FOO"]}, 200)
        req_mock = mock.MagicMock()
        req_mock.get_response.return_value = resp
        m.return_value = req_mock
        ret = self.helper.get_network(None,1)
    ...
    def test_get_network_with_subnet2(self, m):
        ...
        self.assertEqual([2], ret["subnets"])
        self.assertEqual(["FOO"], ret["subnet_info"])
        m.assert_called_with(req_mock, method="GET", path="/subnets/2", query_string=None)

    @mock.patch.object(OpenStackNet, "_make_create_request")
    def test_create_only_network(self, m):
        name ="name_net"; net_id =1; state="ACTIVE"; project="project_id";
        parameters = {"occi.core.title":name,
                       "occi.core.id": net_id,
                       "occi.network.state":state,
                       "project":project,
                     }
        resp = fakes.create_fake_json_resp({"network": {"network_id": net_id}}, 201)
        req_mock = mock.MagicMock()
        req_mock.get_response.return_value = resp
        m.return_value = req_mock
    ...
    def test_create_full_network(self, m):
        name ="name_net"; net_id =1; state="ACTIVE"; project="project_id";
        ip_version="IPv4"; cidr = "0.0.0.0"; gate_way ="0.0.0.1";subnet_id=11
        parameters = {"occi.core.title":name,
                       "occi.core.id": net_id,
                       "occi.network.state":state,
        ...
        resp = fakes.create_fake_json_resp({"network": {"id": net_id}, "subnet":{"id":subnet_id}}, 201)
        req_mock = mock.MagicMock()
        req_mock.get_response.return_value = resp
        m.return_value = req_mock
        ret = self.helper.create_network(None, parameters)
    ...
    def test_create_full_network_2(self, m):
        ...
        ret2 = self.helper.create_subnet(None, parameters)
        self.assertEqual(subnet_id, ret2["id"])
        param = parsers.translate_parameters(self.translation["subnet"], parameters)
        m.assert_called_with(None,  path="/subnets", content_type="application/json", body=json.dumps(parsers.make_body("subnet",param)), method="POST")


    @mock.patch.object(OpenStackNet, "_make_delete_request")
